...
import { MutationRateLimiter, ensureStringifiedMaxMessageSize, } from './sessionrecording-utils';
...
let inited = false;
let tracer;
let lastKnownSession;
let sessionStartTime = 0;
let paused = false;
let eventCounter = 1;
let logCounter = 1;
let mutationRateLimiter;
...
const RumRecorder = {
    ...
    init(config) {
        ...
        if (!(tracerProvider === null || tracerProvider === void 0 ? void 0 : tracerProvider.resource)) {
            console.error('RUM OTEL Web must be inited before recorder.');
            return;
        }
        const resource = tracerProvider.resource;
        const { apiKey, url, debug } = config, rrwebConf = __rest(config, ["apiKey", "url", "debug"]);
        ...
        span.end();
        const exportUrl = url !== null && url !== void 0 ? url : 'https://in-otel.hyperdx.io/v1/logs';
        const headers = {};
        if (apiKey) {
        ...
        lastKnownSession = resource.attributes['rum.sessionId'];
        sessionStartTime = Date.now();
        mutationRateLimiter =
            mutationRateLimiter !== null && mutationRateLimiter !== void 0 ? mutationRateLimiter : new MutationRateLimiter(record, {
        ...
        inited = record(Object.assign(Object.assign({ maskAllInputs: true, maskTextSelector: '*' }, rrwebConf), { emit(srcEvent) {
                if (paused) {
                    return;
                }
                ...
                const body = encoder.encode(ensureStringifiedMaxMessageSize(JSON.stringify(event)));
