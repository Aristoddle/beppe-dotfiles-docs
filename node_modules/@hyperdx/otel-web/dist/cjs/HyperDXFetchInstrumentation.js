"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.HyperDXFetchInstrumentation = void 0;
const instrumentation_fetch_1 = require("@opentelemetry/instrumentation-fetch");
const servertiming_1 = require("./servertiming");
const utils_1 = require("./utils");
// not used yet
async function readStream(stream) {
    const chunks = [];
    const queuingStrategy = new CountQueuingStrategy({ highWaterMark: 1 });
    const decoder = new TextDecoder();
    try {
        await stream.pipeTo(new WritableStream({
            write(chunk) {
                chunks.push(decoder.decode(chunk, { stream: true }));
            },
        }, queuingStrategy));
    }
    catch (e) {
        // Ignore
    }
    finally {
        return chunks.join('');
    }
}
class HyperDXFetchInstrumentation extends instrumentation_fetch_1.FetchInstrumentation {
    constructor(config = {}) {
        const origCustomAttrs = config.applyCustomAttributesOnSpan;
        config.applyCustomAttributesOnSpan = function (span, request, response) {
            var _a;
            // Temporary return to old span name until cleared by backend
            span.updateName(`HTTP ${(request.method || 'GET').toUpperCase()}`);
            span.setAttribute('component', 'fetch');
            if (((_a = config.advancedNetworkCapture) === null || _a === void 0 ? void 0 : _a.call(config)) && span) {
                if (request.headers) {
                    (0, utils_1.headerCapture)('request', Object.keys(request.headers))(span, (header) => { var _a; return (_a = request.headers) === null || _a === void 0 ? void 0 : _a[header]; });
                }
                if (request.body) {
                    if (request.body instanceof ReadableStream) {
                        span.setAttribute('http.request.body', '[ReadableStream]');
                        // FIXME: This is not working yet
                        // readStream(request.body).then((body) => {
                        //   span.setAttribute('http.request.body', body);
                        // });
                    }
                    else {
                        span.setAttribute('http.request.body', request.body.toString());
                    }
                }
                if (response instanceof Response) {
                    if (response.headers) {
                        const headerNames = [];
                        response.headers.forEach((value, name) => {
                            headerNames.push(name);
                        });
                        (0, utils_1.headerCapture)('response', headerNames)(span, (header) => { var _a; return (_a = response.headers.get(header)) !== null && _a !== void 0 ? _a : ''; });
                    }
                    response
                        .clone()
                        .text()
                        .then((body) => {
                        span.setAttribute('http.response.body', body);
                    })
                        .catch(() => {
                        // Ignore
                    });
                }
            }
            if (span && response instanceof Response && response.headers) {
                const st = response.headers.get('Server-Timing');
                if (st) {
                    (0, servertiming_1.captureTraceParent)(st, span);
                }
            }
            if (origCustomAttrs) {
                origCustomAttrs(span, request, response);
            }
        };
        super(config);
    }
    enable() {
        // Don't attempt in browsers where there's no fetch API
        if (!window.fetch) {
            return;
        }
        super.enable();
    }
}
exports.HyperDXFetchInstrumentation = HyperDXFetchInstrumentation;
//# sourceMappingURL=HyperDXFetchInstrumentation.js.map