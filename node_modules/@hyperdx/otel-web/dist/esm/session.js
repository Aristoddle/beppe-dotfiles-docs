...
export const COOKIE_NAME = '__rum_sid';
let rumSessionId;
...
function parseCookieToSessionState() {
    ...
    // startTime validity
    if (!ss.startTime ||
        typeof ss.startTime !== 'number' ||
        pastMaxAge(ss.startTime)) {
        return undefined;
    }
    return ss;
}
...
export function updateSessionStatus() {
    ...
    eventTarget === null || eventTarget === void 0 ? void 0 : eventTarget.emit('session-changed', { sessionId: rumSessionId });
    if (recentActivity) {
        renewCookieTimeout(sessionState);
    }
    recentActivity = false;
}
export function initSessionTracking(instanceId, newEventTarget, domain) {
    if (domain) {
        cookieDomain = domain;
    }
    ...
    [
        ...
        'touchend',
        'visibilitychange',
    ].forEach((type) => document.addEventListener(type, markActivity, {
        capture: true,
        passive: true,
    }));
